...
const AccountSearch = function({ searchOpt, userSource, getInvoiceSourceUrl, gitReviewerUrl, isAddedTax, onChangeSearch, onSearch, emptyClick, loading }) {
    // 分众添加的搜索条件
    const [showMore, setShowMore] = useState(false);
    const { expenseSystemSource, expenseReviewer } = searchOpt;
    const otherItems = (
        <>
    ...
    const isEleTag = searchOpt.invoiceType == 28 || searchOpt.invoiceType == 29;
    return (
        <div className='topBtns'>
            <div className={!showMore ? 'row minHeight' : 'row'} style={{ maxWidth: 1300, display: 'inline-block' }}>
                ...
                <div className={isAddedTax || isEleTag ? 'searchItem inlineBlock' : 'searchItem inlineBlock hidden'}>
                    <label>发票代码：</label>
                    <Input
                        placeholder='输入发票代码'
                        type='text'
            ...
            <div style={{ display: 'inline-block', verticalAlign: 'top' }}>
                <Tooltip title='采集日期过滤查询不到发票时，建议使用开票日期过滤查询'>
                    <Button type='primary' onClick={onSearch} disabled={loading}>查询</Button>
                </Tooltip>
                <Button className='btn' icon='reload' onClick={emptyClick} style={{ borderStyle: 'none', marginLeft: 15 }} disabled={loading}>清空</Button>
            </div>
