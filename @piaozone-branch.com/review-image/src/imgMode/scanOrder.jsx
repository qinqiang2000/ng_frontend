                    ...
                    >
                        ...
                        <div style={{ display: foldList.includes(item.fscanBillNos) ? 'none' : ''}}>
                        {
                            ...
                            {/* <React.Fragment key={item.key}>
                                <div className='mode-subtitle' onClick={() => changeFoldList(`${item.fscanBillNos}${item.key}`)}>
                                    <div className={!foldList.includes(`${item.fscanBillNos}${item.key}`) ? 'icon activate' : 'icon'} />
                                    影像（{item.detailInfos.length}）份
                                </div>
                                {
                                    <div style={{ display: foldList.includes(`${item.fscanBillNos}`) ? 'none' : ''}}>
                                        {
                                            item.detailInfos.map((val, index) => (
                                                <SnapshotItem
                                                    key={val.serialNo}
                                                    mode={mode}
                                                    info={val}
