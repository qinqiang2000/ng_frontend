...
import { invoiceTypeOptions, ApiInvoiceType } from '@/client/components/ui/InvoiceTypeSelector';
...
import { useAuditedRulesTranslation } from '@/client/hooks/useTranslation';
...
function AuditedRulesDetailContent() {
  const params = useParams();
  const searchParams = useSearchParams();
  const { selectedCountry, setSelectedCountry } = useCountry();
  const { showSuccess, showError } = useToast();
  const { t } = useAuditedRulesTranslation();

  // URL参数处理
  const ruleId = params.id as string;
  const urlRuleCodes = searchParams.get('ruleCodes');
  const urlCountry = searchParams.get('country');

  const [activeRule, setActiveRule] = useState(''); // 格式: groupCode:ruleId 或 ruleCode
  // 移除未使用的状态变量
  ...
  // 生成规则分类结构 - 基于groupCode分组
  const generateRuleCategories = useCallback((rules: ApiRule[]) => {
    // 创建基于groupCode的动态分类Map
    const categoryMap = new Map<string, RuleCategory>();

    // 将规则分配到对应的分类中
    rules.forEach(rule => {
      // 使用groupCode进行分组，如果没有groupCode或为空则使用'General'
      const groupCode = (rule as any).groupCode || 'General';
      const categoryLabel = groupCode;

      // 获取或创建分类
       let targetCategory = categoryMap.get(groupCode);
      if (!targetCategory) {
        targetCategory = {
          label: categoryLabel,
          value: groupCode,
          icon: getCategoryIcon(rule.invoiceType || ''),
          count: 0,
          rules: [],
          expanded: false
        };
        categoryMap.set(groupCode, targetCategory);
      }
    ...
    setRuleCategories(prevCategories => {
      ...
      const categoriesWithExpanded = categories.map(c => {
        const containsActive = c.rules.some(r => {
          const groupCode = (r as any).groupCode || 'General';
          const ruleKey = `${groupCode}:${r.id}`;
          return ruleKey === activeRule || r.ruleCode === activeRule;
        });
    ...
    setRuleCategories(prevCategories => {
      return prevCategories.map(category => {
        ...
        const containsRule = category.rules.some(r => {
          const groupCode = (r as any).groupCode || 'General';
          const uniqueKey = `${groupCode}:${r.id}`;
          return (ruleKey.ruleCode && r.ruleCode === ruleKey.ruleCode) ||
                 (ruleKey.id && r.id === ruleKey.id) ||
                 uniqueKey === (ruleKey as any).uniqueKey;
        });
  ...
  const reloadData = useCallback(async () => {
    try {
      ...
      // 如果有URL参数ruleCodes，添加到请求中
      if (urlRuleCodes) {
        requestBody.ruleCodes = urlRuleCodes;
      }

      // 重新加载规则列表
      const response = await fetch(getApiBasePath('/invoice-rules/subscribed-rules-with-groups'), {
        method: 'POST',
        headers: {
        ...
        if (result.errcode === '0000' && result.data) {
          ...
          if (currentRuleDetail) {
            ...
            if (matched) {
              ...
              const matchedGroupCode = (matched as any).groupCode || 'General';
              const uniqueKey = `${matchedGroupCode}:${matched.id}`;
              setActiveRule(uniqueKey);
              setTimeout(() => {
            ...
            try {
              ...
              if (detailResponse.ok) {
                ...
                if (detailResult.errcode === '0000' && detailResult.data) {
                  ...
                  setAiPrompt(ruleDetail.aiPrompt || '');
                }
              }
            } catch (error) {
  ...
  }, [selectedCountry, generateRuleCategories, currentRuleDetail, expandCategoryForRule, toInputDateTime, urlRuleCodes]);
  ...
  const saveRule = useCallback(async (showSuccessMessage = true) => {
    try {
      ...
      const ruleData = {
        ...
        invoiceType: '',
        subInvoiceType: '',
        ruleCode: currentRuleDetail?.ruleCode || activeRule,
        ruleName: ruleName,
        ruleType: 1, // Fixed: validation rules
  ...
  const fetchRules = useCallback(async () => {
    try {
      ...
      // 如果URL参数中存在ruleCodes，添加到请求中进行过滤
      if (urlRuleCodes) {
        requestBody.ruleCodes = urlRuleCodes;
      }
      ...
      if (result.errcode === '0000' && result.data) {
        ...
        // 尝试根据URL中的ID找到对应的规则
        const targetRule = result.data.find(r => r.id.toString() === ruleId || r.ruleCode === ruleId);

        if (targetRule) {
          // 找到指定的规则，选中它
          const targetGroupCode = (targetRule as any).groupCode || 'General';
          const targetUniqueKey = `${targetGroupCode}:${targetRule.id}`;
          setActiveRule(targetUniqueKey);
          setTimeout(() => {
            generateRuleCategories(result.data);
          ...
          if (result.data.length > 0) {
            ...
            const firstRuleGroupCode = (firstRule as any).groupCode || 'General';
            const firstRuleUniqueKey = `${firstRuleGroupCode}:${firstRule.id}`;
            setActiveRule(firstRuleUniqueKey);
            // 显示提示信息
            showError('Rule Not Found', `Rule with ID "${ruleId}" not found. Automatically selected the first rule.`);
            setTimeout(() => {
              generateRuleCategories(result.data);
  ...
  }, [selectedCountry, generateRuleCategories, ruleId, urlRuleCodes, showError]);
  ...
  const handleSearch = useCallback((term: string) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      if (term.trim()) {
        ...
        // 清空搜索时，展开当前选中规则所在的分类
        const updatedCategories = ruleCategories.map(category => {
          const containsActiveRule = category.rules.some(r => {
            const groupCode = (r as any).groupCode || 'General';
            const ruleKey = `${groupCode}:${r.id}`;
            return ruleKey === activeRule || r.ruleCode === activeRule;
          });
  ...
  }, [urlCountry, selectedCountry, setSelectedCountry]);
          ...
          {!leftSidebarCollapsed && (
            <div className="p-4 border-b border-gray-200">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900">Audited Rules Engine</h2>
                <button
                  onClick={addNewRule}
                ...
                {ruleCategories.map((category, index) => {
                  ...
                  >
                    ...
                    <div className="ml-6 space-y-1">
                          {matches.map((rule) => (
                        <button
                              key={`${(rule as any).groupCode || 'General'}:${rule.id}`}
                              onClick={() => {
                                const groupCode = (rule as any).groupCode || 'General';
                                const uniqueKey = `${groupCode}:${rule.id}`;
                                setActiveRule(uniqueKey);
                          ...
                          className={`w-full flex items-center space-x-2 px-3 py-2 rounded-lg text-left cursor-pointer ${
                                (() => {
                                  const groupCode = (rule as any).groupCode || 'General';
                                  const uniqueKey = `${groupCode}:${rule.id}`;
                                  return uniqueKey === activeRule ? 'bg-blue-50 border border-blue-200' : 'hover:bg-gray-100';
                                })()
                          }`}
                        >
                          <div className="w-4 h-4 flex items-center justify-center">
                                <i className={`ri-file-add-line ${
                                  (() => {
                                    const groupCode = (rule as any).groupCode || 'General';
                                    const uniqueKey = `${groupCode}:${rule.id}`;
                                    return uniqueKey === activeRule ? 'text-blue-600' : 'text-gray-500';
                                  })()
                                }`}></i>
                          </div>
                          <div className="flex-1">
                                <div className={`text-sm font-medium ${
                                  (() => {
                                    const groupCode = (rule as any).groupCode || 'General';
                                    const uniqueKey = `${groupCode}:${rule.id}`;
                                    return uniqueKey === activeRule ? 'text-blue-600' : 'text-gray-900';
                                  })()
                                }`}>{rule.ruleCode}</div>
                                <div className="text-xs text-gray-500">{rule.ruleName}</div>
                          </div>
                              <div className={`w-2 h-2 rounded-full ${rule.status === 4 ? 'bg-green-400' : 'bg-gray-400'}`}></div>
                        </button>
                      ))}
                          {matches.length === 0 && (
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col relative">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Rule Prompt</label>
                      <textarea
                        rows={3}
                        value={aiPrompt}
                        onChange={(e) => setAiPrompt(e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder={ruleExpression.trim() !== '' ? "Prompt only takes effect when Rule Expression is empty" : "Enter AI prompt for this rule"}
                      />
                    </div>
...
function AuditedRulesDetailLoading() {
  return (
...
export default function AuditedRulesDetailPage() {
  return (
    <Suspense fallback={<AuditedRulesDetailLoading />}>
        <AuditedRulesDetailContent />
    </Suspense>
  );
}