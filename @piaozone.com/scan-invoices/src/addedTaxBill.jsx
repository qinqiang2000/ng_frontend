...
//未识别或未查验成功的增值税发票
class AddedTaxBill extends BaseBill {
    constructor(props) {
        super(...arguments);
    ...
    onSave = async() => {
        ...
        let invoiceAmount = billData.invoiceAmount;
        // 二手车发票需要车价合计进行查验
        if (this.props.invoiceType == 13) {
            invoiceAmount = billData.totalAmount;
        }
    ...
    render() {
        ...
        let checkCode = billData.checkCode;
        if (!checkCode) {
            checkCode = '';
        }
                    ...
                    <div>
                        ...
                        <div className='inputItems' ref='inputItems'>
                            ...
                            <div className='clearfix'>
                                ...
                                <div className='inputItem floatLeft paddLeft90'>
                                    <label className='require'>{invoiceType !== 13 ? '合计金额：' : '车价合计'}</label>
                                    <Input
                                        type='text'
                                        disabled={disabledEdit}
                                        value={totalAmount}
                                    ...
                                    invoiceType !== 13 ? (
                                        ...
                                        <div className='inputItem floatLeft paddLeft90'>
                                            ...
                                            <Input
                                                type='text'
                                                disabled={disabledEdit}
                                                value={billData.buyerIdNo}
                                                onChange={(e) => this.setState({
                                                    billData: {
                                                        ...billData,
                                                        buyerIdNo: e.target.value.trim()
                                                    }
                                                })}
                                            />
                                        </div>
                            ...
                            <div className='clearfix'>
                                    ...
                                    invoiceType !== 13 ? (
                                        ...
                                        <div className='inputItem floatLeft paddLeft90'>
                                            ...
                                            <Input
                                                disabled={disabledEdit}
                                                type='text'
                                                value={billData.salerIdNo}
                                                onChange={(e) => this.setState({
                                                    billData: {
                                                        ...billData,
                                                        salerIdNo: e.target.value.trim()
                                                    }
                                                })}
                                            />
                                        </div>
                                ...
                                {
                                    invoiceType != 2 && invoiceType != 4 && invoiceType != 12 && invoiceType != 13 ? (
                                        <div className='inputItem floatLeft paddLeft90'>
                                            <label className='require'>校验码后六位：</label>
                                            <Input
                                                maxLength={6}
                                                disabled={disabledEdit}
                                                type='text'
                                                value={checkCode.length <= 6 ? checkCode : checkCode.substr(checkCode.length - 6, 6)}
                                                onChange={(e) => this.setState({
                                                    billData: { ...billData, checkCode: e.target.value.trim(), jym: e.target.value.trim() }
                                                })}
                                            />
                                        </div>
                                    ) : null
