    ...
    startUpload = async(waitFile, onStepFinish, isUseBackend) => {
        ...
        if (isLevelBroser || isUseBackend || !window.URL || !window.URL.createObjectURL) {
            ...
            if (waitFile instanceof File) {
                ...
                // 浏览器处理失败，直接后台处理
                if (res.errcode !== '0000') {
                    const res2 = await this.startAnalysisFullPdf(waitFile, onStepFinish);
                    result.push(res2);
                } else {
                    result.push(res);
                }
                ...
                for (let i = 0; i < waitFile.length; i++) {
                    ...
                    // 浏览器处理失败，直接后台处理
                    if (res.errcode !== '0000') {
                        const res2 = await this.startAnalysisFullPdf(waitFile[i], onStepFinish);
                        result.push(res2);
                    } else {
                        result.push(res);
                    }
