...
export function stepOneInit() {
    ...
    // 关闭评价
    const miniTools = document.querySelectorAll('#logoutmodal .close span');
    ...
    // 关闭公告
    if (document.querySelector('.zj-big-bg .zj-alert-header span')) {
        document.querySelector('.zj-big-bg .zj-alert-header span').click();
        return false;
    }
    // 登录按钮
...
export function waitLoginRedirect() {
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('#menuwrapper a');
    if (!wybsItems.length) {
        return false;
    }
    ...
    const companyName = document.querySelector('#h-tyshxydm');
    return {
        errcode: '0000',
        description: 'success',
        data: {
...
export function selectRole(opt = {}) {
    ...
    // 进入账户中心页面，未进入菜单页面。
    // const userInfo = document.querySelector('.sidebar-head')?.innerText || '';
    // const gobackBtns = document.querySelector('.btns .el-button.el-button--text');
    // const gobackText = gobackBtns?.innerText || '';
    // if (userInfo && (userInfo.indexOf('账户中心') !== -1) && gobackBtns && (gobackText.indexOf('返回') !== -1)) {
    //     document.querySelector('.btns .el-button.el-button--text').click();
    //     return {
    //         errcode: '0000',
    //         data: true
    //     };
...
export function apiWaitLogin() {
    // 已经是登录状态
    if (document.querySelector('.css-sub-menu .sub-menu-nav')) {
        return {
            errcode: '0000',
            data: {
                isLogined: true
