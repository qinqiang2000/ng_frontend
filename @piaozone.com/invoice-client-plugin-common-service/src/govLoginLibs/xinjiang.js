...
export async function stepOneInit() {
    ...
    // 温馨提示
    const tipDoms = document.querySelectorAll('.mini-panel');
    if (tipDoms.length) {
        // 检测是否全部关闭
        for (const tipDom of tipDoms) {
            if (tipDom.style.display !== 'none') {
                tipDom.querySelector('.mini-tools-close ').click();
            }
        }
    }
    // 登录按钮
    const loginDoms = document.querySelectorAll('.menu-login-new .newLogin');
    if (loginDoms.length) {
        for (const loginDom of loginDoms) {
            if (~loginDom.innerText.indexOf('登录')) {
                const url = loginDom.getAttribute('href');
...
export function waitLoginRedirect() {
    ...
    const msgModal = document.querySelector('.layui-layer-btn0');
    if (msgModal) {
        msgModal.click();
    }
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('#wybs-content li a');
    if (!wybsItems.length) {
        const obj = document.querySelector('.newLogin');
        if (obj) {
    ...
    const companyName = document.querySelector('.operate-left p');
    return {
        errcode: '0000',
        description: 'success',
        data: {
            companyName: companyName ? companyName.getAttribute('title') : '',
...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    if (checkClickBtn) {
        if (document.querySelector('.right-menu .newLogin')) {
            document.querySelector('.right-menu .newLogin').removeAttribute('target');
            return {
                errcode: '0000',
                data: {
                    selector: '.right-menu .newLogin'
                }
            };
        }
    }
