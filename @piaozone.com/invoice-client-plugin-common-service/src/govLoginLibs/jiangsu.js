...
export async function stepOneInit() {
    ...
    const wybsItem = document.querySelector('.tabs_d .wybs_tab');
    if (wybsItem) {
        return {
            errcode: '0000',
            description: 'success',
    ...
    // 关闭公告
    if (document.querySelector('.xubox_layer .xubox_close')) {
        document.querySelector('.xubox_layer .xubox_close').click();
        return false;
    }
    // 登录按钮
    if (document.querySelector('.list.kx_xbdl')) {
        document.querySelector('.list.kx_xbdl').click();
        return false;
    }
    return false;
}
...
export function waitLoginRedirect() {
    // 登录完成后首页的弹窗
    const btn0 = document.querySelector('.layui-layer-btn0');
    if (btn0) {
        btn0.click();
    }
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('.wybs .block');
    if (!wybsItems.length) {
        return false;
    }
    ...
    const companyName = document.querySelector('#span_user');
    return {
        errcode: '0000',
        description: 'success',
        data: {
            companyName: companyName ? (companyName.innerText && companyName.innerText.replace('欢迎, ', '')) : '',
...
// 点击防诈骗按钮
export function oauthClick() {
    const oauth_a = document.querySelector('.bottom>.oauth_a a');
    if (oauth_a) {
        oauth_a.click();
    }
    return false;
...
// 检测有无防诈骗弹窗
export function checkFraudPrevention() {
    if (document.querySelector('.bottom>.oauth_a a')) {
        return {
            errcode: '0000',
            data: true
        };
...
export function apiWaitLogin() {
    // 已经是登录状态
    if (document.querySelector('.tabs_d .wybs_tab')) {
        return {
            errcode: '0000',
            data: {
                isLogined: true
    ...
    if (typeof window.openKxLogin !== 'function') {
        return false;
    }
