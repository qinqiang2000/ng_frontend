...
export async function stepOneInit() {
    ...
    const wybsItem = document.querySelector('#top li[linktype=wybs]');
    if (wybsItem) {
        return {
            errcode: '0000',
            description: 'success',
    ...
    // 温馨提示
    if (document.querySelector('.layui-layer .layui-layer-close')) {
        document.querySelector('.layui-layer .layui-layer-close').click();
        return false;
    }
    // 登录按钮
    if (document.querySelector('#kxLogin .pages-login-button')) {
        document.querySelector('#kxLogin .pages-login-button').click();
        return false;
    }
    return false;
}
...
export function waitLoginRedirect() {
    ...
    const infoBtn = document.querySelector('.index-step').children;
    if (infoBtn && infoBtn.length > 0) {
        Array.from(infoBtn).forEach((item) => item.querySelector('a') && item.querySelector('a').click());
    }
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('#top .bd .pages-index-entry:nth-child(2) li');
    if (!wybsItems.length) {
        return false;
    }
    ...
    const companyName = document.querySelector('.unit span');
    return {
        errcode: '0000',
        description: 'success',
        data: {
    ...
    if (checkClickBtn) {
        if (document.querySelector('#kxLogin .pages-login-button')) {
            return {
                errcode: '0000',
                data: {
                    selector: '#kxLogin .pages-login-button'
                }
            };
        }
    }
...
export function apiWaitLogin() {
    // 已经是登录状态
    if (document.querySelector('#top li[linktype=wybs]')) {
        return {
            errcode: '0000',
            data: {
                isLogined: true
    ...
    const obj = document.querySelector('#kxLogin a');
    if (obj) {
        const loginUrl = obj.getAttribute('href');
        if (loginUrl.indexOf('http') !== -1) {
            return {
                errcode: '0000',
                data: {
                    loginUrl: loginUrl
