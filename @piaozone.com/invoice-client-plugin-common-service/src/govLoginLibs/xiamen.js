...
export function stepOneInit() {
    ...
    if (document.querySelector('#kxsfDldz')) {
        document.querySelector('#kxsfDldz').click();
        return false;
    }
...
export function waitLoginRedirect() {
    ...
    // 执行我要办税函数
    $('#wybsTab').click();
    const wybsItems = $('#topTabs .layui-tab-content .layui-tab-item').eq(2).find('.itme-icon');
    ...
    const c1 = myGetCookie('route');
    const companyName = $('#userName2').text();
    if (c1) {
        return {
            errcode: '0000',
            description: 'success',
...
export function waitOpenTreeNode(opt = {}) {
    ...
    const target = $('.layui-tab-content .layui-tab-item').eq(2).find('.itme-icon').eq(opt.index).find('a');
    if (target.length > 0) {
        window.location.href = target.attr('onclick').split('",')[0].replace('goIndexUrl("', '');
        return {
            errcode: '0000',
            data: true
        };
...
export function waitSzzhTreeNode(opt = {}) {
    ...
    const target = $('.layui-tab-content .layui-tab-item').eq(2).find('.itme-icon').eq(opt.index).find('a');
    if (target.length > 0) {
        window.location.href = target.attr('onclick').split('",')[0].replace('openUrl("', '');
        return {
            errcode: '0000',
            data: true
        };
