...
export function stepOneInit() {
    ...
    const info = document.querySelector('#fd_div_ningxia .pop_div_title_close');
    if (info) {
        info.click();
    }

    const loginico = document.querySelector('.loing');
    if (loginico) {
        loginico.click();
    }
    return false;
...
export function waitLoginRedirect() {
    ...
    let szzhItem;
    // 获取权限
    const wybsItems = document.querySelectorAll('#myTabContent #wybs .wybs_line a');
    if (!wybsItems.length) {
        return false;
    }
    ...
    const companyName = document.querySelector('#yhmc')?.title || '';
    // 获取公司名称
    if (companyName) {
        return {
            errcode: '0000',
