...
export async function stepOneInit() {
    ...
    // 温馨提示
    const tipDoms = document.querySelectorAll('.layui-layer-iframe .layui-layer-close');
    if (tipDoms.length) {
    ...
    // 登录按钮
    const loginDom = document.querySelector('.dlbox .loginico');
    if (loginDom) {
        loginDom.click();
    }
    return false;
...
export function waitLoginRedirect() {
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('.layui-tab-content .layui-tab-item:nth-child(3) .itme-icon');
    if (!wybsItems.length) {
        return false;
    }
