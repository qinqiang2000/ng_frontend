...
export function stepOneInit() {
    ...
    const info = document.querySelector('.layui-layer-setwin a');
    if (info) {
        info.click();
    }

    const loginico = document.querySelector('.dlbox a');
    if (loginico) {
        loginico.click();
    }
    return false;
...
export async function checkStepOneLogin(data = {}) {
    ...
    // 我要办税是否出现
    // 直接登录成功
    const wybs = document.querySelector('#wybsTab');
    if (wybs) {
        return {
            errcode: '0000',
            description: 'success',
...
export async function waitLoginRedirect() {
    const msgObj = document.querySelector('.el-message.el-message--warning');
    if (msgObj && msgObj.innerText) {
        return {
            errcode: '0000',
    ...
    // 等待我要办税下的菜单加载
    await window.pwyNightmare.sleep(1500);

    let openInvoiceItem;
    let szzhItem;
    const wybsItems = document.querySelector('.layui-show .conbox .layui-row')?.children || [];
    ...
    const companyName = document.querySelector('#userName2').innerText;
    // 获取公司名称
    if (companyName) {
        return {
            errcode: '0000',
...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    if (document.querySelector('.layui-show .conbox .layui-row')) {
        // 已经是登录状态
        const wybsItems = document.querySelector('.layui-show .conbox .layui-row')?.children || [];
