...
export async function stepOneInit() {
    ...
    const wybsItem = document.querySelector('#mine-ul');
    if (wybsItem) {
        return {
            errcode: '0000',
            description: 'success',
...
export function waitLoginRedirect() {
    ...
    const wybsItem = document.querySelector('#mine-ul');
    if (!wybsItem) {
        const loginDom = document.querySelector('.help-and-login .login');
        if (loginDom) {
            loginDom.click();
            return false;
        }
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('#panel-22020 li a');
    if (!wybsItems.length) {
        return false;
    }
    ...
    const companyName = document.querySelector('.user-name');
    return {
        errcode: '0000',
        description: 'success',
        data: {
...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    if (checkClickBtn) {
        const btns = document.querySelector('.help-and-login .login');
        if (btns) {
            return {
                errcode: '0000',
                data: {
                    selector: '.help-and-login .login'
                }
            };
        }
    }
...
export function apiWaitLogin() {
    // 已经是登录状态
    if (document.querySelector('#mine-ul')) {
        return {
            errcode: '0000',
            data: {
                isLogined: true
