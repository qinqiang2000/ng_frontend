...
export function stepOneInit() {
    ...
    if (document.querySelector('.loginico')) {
        document.querySelector('.loginico').click();
        return false;
    }
    return false;
}
...
export function waitLoginRedirect() {
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('.layui-tab-content .layui-tab-item:nth-child(3) .layui-row .itme-icon');
    if (!wybsItems.length) {
        return false;
    }
...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    if (checkClickBtn) {
        const button = document.querySelector('.loginico');
        if (button) {
            return {
                errcode: '0000',
                data: {
...
export function apiWaitLogin() {
    ...
    const obj = document.querySelector('#kxLoginUrl');
    if (!obj) {
        return false;
    }
