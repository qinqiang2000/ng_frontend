...
export async function stepOneInit() {
    ...
    if (document.querySelector('#newLogin .login')) {
        ...
        document.querySelector('#newLogin .login').click();
        return false;
    }
    return false;
}
...
export function waitLoginRedirect() {
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('#wybs-content .menu-item');
    if (!wybsItems.length) {
        return false;
    }
    ...
    const companyName = document.querySelector('.account-operate .operate-left p:first-child');
    return {
        errcode: '0000',
        description: 'success',
        data: {
            companyName: companyName ? (companyName.innerText && companyName.innerText.replace(/(.*)(（.*）)/, '$1')) : '',
...
export function apiWaitLogin() {
    // 已经是登录状态
    if (document.querySelector('.header-tab .wybsT')) {
        return {
            errcode: '0000',
            data: {
                isLogined: true
