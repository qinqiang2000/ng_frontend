...
export function stepOneInit() {
    ...
    // 关闭提醒
    if (document.querySelector('.modal-header .close')) {
...
export function waitLoginInit() {
    ...
    // 菜单是否出现
    const wybsItem = document.querySelector('#BSZX_tab');
    if (wybsItem) {
        return {
            errcode: '0000',
            description: 'success',
...
export function waitLoginRedirect() {
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('.tab-button span:nth-child(2)');
    if (!wybsItems.length) {
        return false;
    }
    ...
    const companyName = document.querySelector('.user-info');
    return {
        errcode: '0000',
        description: 'success',
        data: {
...
export function apiWaitLogin() {
    // 已经是登录状态
    if (document.querySelector('#BSZX_tab')) {
        return {
            errcode: '0000',
            data: {
                isLogined: true
    ...
    return {
        errcode: '0000',
        data: {
            loginUrl: window.location.origin + window.contextPath + '/kxLogin.c'
        }
    };
}