...
export async function stepOneInit() {
    ...
    // 旧
    const wybsItem = document.querySelector('.css-sub-menu .sub-menu-nav');
    // 新
    const wybsItemNew = document.querySelector('#taxs .sub-menu-nav');
    if (wybsItem || wybsItemNew) {
        return {
            errcode: '0000',
            description: 'success',
            data: {
    ...
    // 温馨提示
    if (document.querySelector('.zj-big-bg .confirm')) {
        document.querySelector('.zj-big-bg .confirm').click();
        return false;
    }
    // 登录退出
    if (document.querySelector('#tjbutton')) {
        document.querySelector('#tjbutton').click();
        return false;
    }
    // 登录按钮
...
export function waitLoginRedirect() {
    // 设置session避免弹窗
    window.sessionStorage.handleAlerts = 'Y';
    ...
    // 税务抵达文书
    if (document.querySelector('#readedyhxy')) {
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('#menuwrapper .sub-menu-item');
    // 新
    const wybsItemsNew = document.querySelectorAll('#taxs .func-menu-item > span');
    if (!wybsItems.length && !wybsItemsNew.length) {
        return false;
    }

    let openInvoiceItem;
    let szzhItem;
    const wybsItemsArray = wybsItems.length ? wybsItems : wybsItemsNew;
    for (let i = 0; i < wybsItemsArray.length; i++) {
        const text = wybsItemsArray[i].innerText || '';
    ...
    const companyName = document.querySelector('#homelink').nextSibling;
    return {
        errcode: '0000',
        description: 'success',
        data: {
...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    // 清除弹窗
    if (document.querySelector('.layui-layer-dialog .layui-layer-close')) {
    ...
    if (checkClickBtn) {
        if (document.querySelector('#newLogin .login')) {
            // 不打开新窗口
            document.querySelector('#newLogin').removeAttribute('target');
            return {
                errcode: '0000',
                data: {
                    selector: '#newLogin .login'
                }
            };
        }
    }
...
export function apiWaitLogin() {
    ...
    if (typeof window.gotoKxLoginPage === 'function') {
        window.gotoKxLoginPage();
    }
    return false;
}