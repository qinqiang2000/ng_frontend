...
export async function checkStepOneLogin(data = {}) {
    ...
    // 我要办税是否出现
    const wybs1 = document.querySelector('#myTab #wybs1 a');
    // 直接登录成功
    if (wybs1) {
        return {
            errcode: '0000',
...
export function waitLoginRedirect() {
    ...
    const wybs1 = document.querySelector('#wybs1 a');
    ...
    const alternateCompanyName = companyNode.innerText?.replace(/,?，/,',').split(',')[1] || '';
    if (companyName || alternateCompanyName) {
        return {
            errcode: '0000',
            description: 'success',
            data: {
                ...
                companyName: companyName || alternateCompanyName,
                companyType: '03'
            }
        };
    }
...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    // 已经是登录状态
    const wybsItem = document.querySelector('#wybs .wybs_line');
    if (wybsItem) {
        return {
            errcode: '0000',
            data: {
    ...
    if (checkClickBtn) {
        if (document.querySelector('.function .function_top #ksLogin')) {
            return {
                errcode: '0000',
                data: {
                    selector: '.function .function_top #ksLogin'
                }
            };
        }
    }
...
export function apiWaitLogin() {
    if (document.querySelector('#wybs .wybs_line')) {
        return {
            errcode: '0000',
            data: {
                isLogined: true
