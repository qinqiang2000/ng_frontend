...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    if (checkClickBtn) {
        const loginObj = document.querySelector('.right-menu .login-btn');
        if (loginObj) {
            return {
                errcode: '0000',
                data: {
                    selector: '.right-menu .login-btn'
                }
            };
        }
    }
...
export function selectRole(opt = {}) {
    // 小规模纳税人
    const allFunBtn = document.querySelector('.header-info .gxh .mr20');
    // 是否出现电子税务局全部功能
    if (allFunBtn) {
        allFunBtn.click();
    }
...
export function waitLoginRedirect() {
    ...
    const tipBoxObj1 = document.querySelector('.layui-layer-dialog .layui-layer-close1');
    if (tipBoxObj1) {
        tipBoxObj1.click();
        return false;
    }
...
export function apiWaitLogin() {
    ...
    if (!window.KX_CLIENTID || !window.KX_URL) {
        return false;
    }

    const n = window.location.origin;
    const t = encodeURIComponent(n + '/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666');
    const e = window.KX_URL + '/api/v1.0/auth/oauth2/login?redirect_uri=' + t + '&client_id=' + window.KX_CLIENTID + '&response_type=code&state=test';
