...
export function stepOneInit() {
    ...
    // 新版登录
    const loginBtn = document.querySelector('#top-login a');
    if (loginBtn) {
        loginBtn.click();
    }
    return false;
...
export async function checkStepOneLogin(data = {}) {
    ...
    // 我要办税是否出现
    // 直接登录成功
    const wybs = document.querySelector('.layui-tab-title')?.children || [];
...
export function waitLoginRedirect() {
    ...
    const loginBtn = document.querySelector('#top-login a');
    if (loginBtn && !wybsItem) {
        loginBtn.click();
        return false;
    }
    if (wybsItem && wybsItem.innerText !== '我要办税') {
        const wybs = document.querySelector('.layui-tab-title')?.children || [];
        for (let i = 0; i < wybs.length; i++) {
            if (wybs[i] && wybs[i].innerText.indexOf('我要办税') !== -1) {
                wybs[i].click();
    ...
    const index = $('.layui-tab-title .layui-this').index();

    // 获取权限
    let openInvoiceItem;
    let szzhItem;
    if ($('.layui-tab-content .tax-ui.layui-show').index() !== index) {
        return false;
    }
    const wybsItems = document.querySelector('.layui-show')?.children || [];
    ...
    const companyName = document.querySelector('#user-name').title;
    // 获取公司名称
    if (companyName) {
        return {
            errcode: '0000',
...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    if (checkClickBtn) {
        if (document.querySelector('#top-login a')) {
            return {
                errcode: '0000',
                data: {
                    selector: '#top-login a'
                }
            };
        }
    }
...
export function apiWaitLogin() {
    ...
    if (typeof window.kxrz === 'function') {
        window.kxrz();
    }
    return false;
}