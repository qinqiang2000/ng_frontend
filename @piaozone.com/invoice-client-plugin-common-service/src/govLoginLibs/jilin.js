...
export async function stepOneInit() {
    ...
    // 公告
    const tipDom = document.querySelector('#EjectBox');
    if (tipDom && tipDom.style.display !== 'none') {
        if (tipDom.querySelector('.pop_div_title_close')) {
            tipDom.querySelector('.pop_div_title_close').click();
            return false;
        }
    }
    // 登录按钮
...
export function waitLoginRedirect() {
    ...
    const wybsItem = document.querySelector('#wybs1');
    if (!wybsItem) {
        if (document.querySelector('#oldLogin .loing')) {
            document.querySelector('#oldLogin .loing').click();
            return false;
        }
        return false;
    }
    ...
    // 根据菜单判断是否有权限
    const wybsItems = document.querySelectorAll('#wybs .wybs_list');
    if (!wybsItems.length) {
        return false;
    }
    ...
    const companyName = document.querySelector('#yhmc');
    return {
        errcode: '0000',
        description: 'success',
        data: {
