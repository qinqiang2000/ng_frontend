...
export async function tryLoginWait({ checkClickBtn }) {
    ...
    if (checkClickBtn) {
        const loginObj = document.querySelector('#kxText');
        if (loginObj) {
            return {
                errcode: '0000',
                data: {
...
export function waitLoginRedirect() {
    ...
    const magCont = document.querySelector('.layui-layer');
    const msgModal = document.querySelector('.layui-layer-shade');
    if (magCont && msgModal) {
        magCont.remove();
        msgModal.remove();
    }
    ...
    const wybsItems = document.querySelectorAll('.layui-row .itme-icon');
