...
import * as govLogins from '../govLoginLibs/chongqing';

const eUrl = 'https://etax.chongqing.chinatax.gov.cn';
const etaxBaseUrl = 'https://dppt.chongqing.chinatax.gov.cn:8443';
const eLoginUrl = 'https://etax.chongqing.chinatax.gov.cn/sword?ctrl=KxLoginCtrl_redirect';
const redirectURI = 'https://etax.chongqing.chinatax.gov.cn/sword?ctrl=KxLoginCtrl_initNsrxx';

export class ChongqingLoginService extends BaseService {
    // 输入
    async stepOneTypes(nt: any, data: any = {}, retry = 0) {
        ...
        try {
            ...
            // 切换短信登录并获取手机号
            await nt.click('.formContent .tabsCls .justifyCenterEnd > div:nth-child(2) > span');
    ...
    async tryLogin(loginData: any) {
        const ctx = this.ctx;
        const { cityPageId, taxNo } = ctx.request.query;
        const ntInfo = ctx.bsWindows.fpdkGovWin;
        let nt = ntInfo ? ntInfo[cityPageId] : null;
