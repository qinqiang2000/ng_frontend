...
import * as govLogins from '../govLoginLibs/shenzhen';

const eUrl = 'https://etax.shenzhen.chinatax.gov.cn';
const etaxBaseUrl = 'https://dppt.shenzhen.chinatax.gov.cn:8443';
const eLoginUrl = 'https://etax.shenzhen.chinatax.gov.cn/bszm-web/apps/views/beforeLogin/indexBefore/pageIndex.html#/';
const redirectURI = eUrl + '/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666';

export class ShenzhenLoginService extends BaseService {
    ...
    async tryLogin(loginData) {
        ...
        if (!nt || !nt.nightmareWindow.win) {
            ctx.service.log.info('login url', eUrl, cityPageId);
