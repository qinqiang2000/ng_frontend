...
import * as govLogins from '../govLoginLibs/qinghai';

const eUrl = 'https://etax.qinghai.chinatax.gov.cn';
const etaxBaseUrl = 'https://dppt.qinghai.chinatax.gov.cn:8443';
const eLoginUrl = 'https://etax.qinghai.chinatax.gov.cn';
const redirectURI = `${eUrl}/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=https%3A%2F%2Fetax.qinghai.chinatax.gov.cn%2Fxxmh%2Fhtml%2Findex_login.html`;

export class QinghaiLoginService extends BaseService {
    ...
    async stepOneTypes(nt: any, data: any = {}, retry = 0) {
        ...
        try {
            ...
            ctx.service.log.info('checkStepOneLogin1', (+new Date()) - startTime, checkRes);
    ...
    async tryLogin(loginData: any) {
        ...
        if (!nt || !nt.nightmareWindow.win) {
            ...
            const ntRes = await ctx.service.ntTools.getEtaxPage(eUrl, {
                ...
                partition: 'persist:' + cityPageId
                // headers: {
                //     cookies: loginData.loginedCookies
                // }
            });
            if (ntRes.errcode !== '0000') {
                return errcodeInfo.govLogout;
