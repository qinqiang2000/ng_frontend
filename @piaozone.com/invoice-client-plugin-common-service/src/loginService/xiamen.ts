...
import * as govLogins from '../govLoginLibs/xiamen';
import { sleep } from '../utils/tools';

const eUrl = 'https://etax.xiamen.chinatax.gov.cn';
const etaxBaseUrl = 'https://dppt.xiamen.chinatax.gov.cn:8443';
const eLoginUrl = 'https://etax.xiamen.chinatax.gov.cn/xxmh/html/index.html';
const redirectURI = `${eUrl}/kxsfrz-cjpt-web/tpass/tpassLogin.do?service=https%3A%2F%2Fetax.xiamen.chinatax.gov.cn%2Fxxmh%2Fhtml%2Findex_login.html`;
export class XiamenLoginService extends BaseService {
    ...
    async stepOneTypes(nt: any, data : any = {}, retry = 0) {
        ...
        let checkRes;
        try {
            const wRes = await nt.wait(govLogins.stepOneInit, {
                waitTimeout: 20000,
                timeoutRefresh: true
            }, 2);
            ctx.service.log.info('init', (+new Date()) - startTime, wRes);
    ...
    async stepOneLogin(opt: any = {}) {
        ...
        ctx.service.log.info('plugin service1 ------');
        const { data = {} } = opt;
        const { taxNo = '', account = '', accountPasswd = '' } = data;
        if (!taxNo || !account || !accountPasswd) {
            return errcodeInfo.argsErr;
