...
import * as govLogins from '../govLoginLibs/qingdao';
import { sleep } from '../utils/tools';


const eUrl = 'https://etax.qingdao.chinatax.gov.cn:7553';
const etaxBaseUrl = 'https://dppt.qingdao.chinatax.gov.cn:8443';
const eLoginUrl = 'https://etax.qingdao.chinatax.gov.cn:7553/sso/login?service=https%3A%2F%2Fetax.qingdao.chinatax.gov.cn%2Fportal%2F';
const redirectURI = eUrl + '/sso/login?service=https://etax.qingdao.chinatax.gov.cn/portal/&qtxtlx=kxlogin';
export class QingdaoLoginService extends BaseService {
    ...
    async stepOneTypes(nt, data = {}, retry = 0) {
        ...
        let checkRes;
        try {
            const initRes = await nt.wait(govLogins.stepOneInit, {
                waitTimeout: 15000,
                timeoutRefresh: true
            }, 2);
            ...
            // startTime = +new Date();
            // const dray2 = await nt.evaluate(govLogins.drayBtn);
            // ctx.service.log.info('drayBtn2', (+new Date()) - startTime, dray2);
        ...
        if (checkRes.errcode !== '0000' || checkData.errMsg) {
            return {
                ...errcodeInfo.argsErr,
                description: checkRes.errcode !== '0000' ? checkRes.description : checkData.errMsg
            };
        }
        return {
            ...checkRes,
    ...
    async sendShortMsg(opt) {
        ...
        ctx.service.log.info('sendShortMsg检查是否在第二次登录状态', (+new Date()) - startTime, stepTwoRes);
        if (!stepTwoRes || stepTwoRes.errcode !== '0000' || stepTwoRes.data?.isLogin) {
            // 非快捷登录
            if (data.isQuickLogin !== true) {
                return errcodeInfo.govLogout;
            }
    ...
    async queryBillMenuIndex(nt, opt) {
        ...
        const { szzhCompany = '', openInvoiceCompany = '', companyName = '', companyType = '03' } = redirectRes.data || {};
        return {
            ...errcodeInfo.success,
            data: {
                companyName,
    ...
    async login(opt) {
        ...
        // 第二步登录可能已经在处于后台，直接判断
        if (nt && nt.nightmareWindow.win) {
            const res = await nt.evaluate(govLogins.waitRedirect);
            if (res && res.errcode === '0000') {
                const res1 = await this.redirectBillModule(nt, {});
                return res1;
            }
        }
