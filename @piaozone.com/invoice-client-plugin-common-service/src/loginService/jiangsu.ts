...
import * as govLogins from '../govLoginLibs/jiangsu';

const eUrl = 'https://etax.jiangsu.chinatax.gov.cn';
const etaxBaseUrl = 'https://dppt.jiangsu.chinatax.gov.cn:8443';
const eLoginUrl = 'https://etax.jiangsu.chinatax.gov.cn/sso/login';
const redirectURI = 'https://etax.jiangsu.chinatax.gov.cn/sso/kxLogin/authorize';

export class JiangsuLoginService extends BaseService {
    ...
    async queryBillMenuIndex(nt: any, opt: any = {}) {
        ...
        // 点击我要办税
        await nt.click('.tabs_d .wybs_tab');
    ...
    async login(opt: any) {
        ...
        const startTime = +new Date();
        // 检测是否有防诈骗窗口----》选择角色前
        const fraudPreventionDataOne = await nt.wait(govLogins.checkFraudPrevention, { waitTimeout: 3500 });

        if (fraudPreventionDataOne && fraudPreventionDataOne.data) {
            // 有则点击
            await nt.evaluate(govLogins.oauthClick);
            ctx.service.log.info('fraudPreventionDataOne', (+new Date() - startTime), fraudPreventionDataOne);
        }
        ...
        // 检测是否有防诈骗窗口 ---》选择角色后
        const fraudPreventionDataTwo = await nt.wait(govLogins.checkFraudPrevention, { waitTimeout: 2000 });
        if (fraudPreventionDataTwo && fraudPreventionDataTwo.data) {
            // 有则点击
            await nt.evaluate(govLogins.oauthClick);
            ctx.service.log.info('fraudPreventionDataTwo', (+new Date() - startTime), fraudPreventionDataTwo);
        }
        const res = await this.etaxLoginSuccess(nt, opt);
        return res;
    }
