...
import * as govLogins from '../govLoginLibs/jilin';

const eUrl = 'https://etax.jilin.chinatax.gov.cn:10812';
const etaxBaseUrl = 'https://dppt.jilin.chinatax.gov.cn:8443';
const eLoginUrl = 'https://etax.jilin.chinatax.gov.cn:10812/sword?ctrl=LoginCtrlTmp_logout';
const redirectURI = 'https://etax.jilin.chinatax.gov.cn:10812/sword?ctrl=DzswjKxtxCtrl_getKxtxOauth2Token';

export class JilinLoginService extends BaseService {
    // 检查登录
    ...
    async login(opt: any) {
        ...
        const { cityPageId } = ctx.request.query;
        const { data = {} } = opt;
        const { shortMsgCode = '', taxNo, roleText = '', shortMsg = '' } = data;
