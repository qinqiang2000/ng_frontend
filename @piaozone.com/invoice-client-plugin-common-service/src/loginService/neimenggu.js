...
import * as govLogins from '../govLoginLibs/neimenggu';
import { sleep } from '../utils/tools';

const eUrl = 'https://etax.neimenggu.chinatax.gov.cn';
const etaxBaseUrl = 'https://dppt.neimenggu.chinatax.gov.cn:8443';
const eLoginUrl = 'https://etax.neimenggu.chinatax.gov.cn/sword?ctrl=LoginCtrlTmp_logout&dlsf=NeiMengGu';
const redirectURI = 'https://etax.neimenggu.chinatax.gov.cn/sword?ctrl=DzswjKxtxCtrl_getKxtxOauth2Token';

export class NeimengguLoginService extends BaseService {
    ...
    async queryBillMenuIndex(nt, opt) {
        ...
        // 弹窗出来慢一些未关闭到，导致后续跳转税务数字账户失败，需单独关闭
        await nt.wait(govLogins.closeModalDialog, {
            waitTimeout: 3000
        });
    ...
    async etaxLoginSuccess(nt, opt) {
        ...
        const { data = {} } = opt;
        const { roleText = '' } = data;
        let res = await this.queryBillMenuIndex(nt, opt);
        ...
        res = await ctx.service.etaxFpdkLogin.common.redirectBillModule(nt, {
            companyName,
            companyType,
            openInvoiceCompany,
            szzhCompany,
