...
const eLoginUrl = 'https://etax.shanghai.chinatax.gov.cn/wszx-web/bszm/apps/views/beforeLogin/indexBefore/pageIndex.html#/';
const redirectURI = `${eUrl}/login-web/api/third/sso/login/redirect?qd=KEXIN&channelId=web&goto=30010666`;

export class ShanghaiV2LoginService extends BaseService {
    ...
    async queryBillMenuIndex(nt, opt) {
        ...
        if (redirectRes.errcode !== '0000' || (redirectRes.data && redirectRes.data.errMsg)) {
            // 旧版
            if (~redirectRes?.data?.errMsg.indexOf('当前会话已失效')) {
                return {
                    ...errcodeInfo.govLogout,
                    description: '登录会话超时，请重新操作登录'
                };
            }
            return {
                ...errcodeInfo.argsErr,
    ...
    async login(opt) {
        ...
        // 检查是否在第二次登录
        const stepTwoRes = await nt.evaluate(govLogins.checkIsStepTwo);
        ctx.service.log.info('检查是否在第二次登录状态', stepTwoRes, (+new Date()) - startTime);
        if (!stepTwoRes || stepTwoRes.errcode !== '0000') {
            return errcodeInfo.govLogout;
        }
