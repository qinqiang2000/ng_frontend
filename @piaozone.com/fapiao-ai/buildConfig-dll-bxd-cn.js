...
const distDir = path.join(__dirname, './dist/bxd-cn');
...
module.exports = {
    ...
    extractCss: true,
    useHash: false,
    library: 'getFapiaoAiBxdCnInstance', // // 指定类库名,主要用于直接引用的方式(比如使用script 标签)
    libraryExport: 'default', //libraryExport: 'default', // 和具体export有关系，如果使用了export default导出模块这里最好使用default，否则不要添加这个配置
    libraryTarget: 'umd', // 定义打包方式Universal Module Definition,同时支持在CommonJS、AMD和全局变量使用
    entry: {
        // fapiaoAi: path.join(srcDir, 'index.js'),
        bxdCn: path.join(srcDir, 'bxdCn.js')
    },
    externals: {
        // react: 'React',
        // 'react-dom': 'ReactDOM'
