...
export class ShaanxiController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.shaanxi.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const res = await ctx.service.etaxFpdkLogin.shaanxi.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        // login 登录状态检测取消
        const res = await ctx.service.etaxFpdkLogin.shaanxi.login({
            data: decryedData,
            needCheck: false
        });
        ...
        // 添加或更新服务端与客户端的电子税局账号
        const opt = {
            city: decryedData.city || '陕西',
            etaxName: decryedData.etaxName || 'shaanxi',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
