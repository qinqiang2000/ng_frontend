...
export class HubeiController extends PluginBaseController {
    async login() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.hubei.login({ data: decryedData });
        if (res.errcode !== '0000') {
        ...
        // 更新客户端与服务端全电账号
        const opt = {
            city: decryedData.city || '湖北',
            etaxName: decryedData.etaxName || 'hubei',
            autoLogin: true,
            taxNo,
            companyName,
            loginAccountUid,
