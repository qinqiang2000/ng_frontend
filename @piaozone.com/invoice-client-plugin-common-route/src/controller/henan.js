...
export class HenanController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        // 参数校验
        ...
        const res = await ctx.service.etaxFpdkLogin.henan.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const { decryedData } = checkRes.data;
        const res = await ctx.service.etaxFpdkLogin.henan.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.henan.login({ data: decryedData });
        if (res.errcode !== '0000') {
        ...
        // 更新客户端与服务端数电账号
        const opt = {
            city: decryedData.city || '河南',
            etaxName: decryedData.etaxName || 'henan',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
