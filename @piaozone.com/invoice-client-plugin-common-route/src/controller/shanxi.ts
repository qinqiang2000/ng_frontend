...
export class ShanxiController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.shanxi.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const { decryedData } = checkRes.data;
        const res = await ctx.service.etaxFpdkLogin.shanxi.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.shanxi.login({ data: decryedData });
        if (res.errcode !== '0000') {
        ...
        // 更新客户端与服务端数电账号
        const opt = {
            city: decryedData.city || '山西',
            etaxName: decryedData.etaxName || 'shanxi',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
