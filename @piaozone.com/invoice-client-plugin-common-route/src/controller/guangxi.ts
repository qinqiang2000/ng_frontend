...
export class GuangxiController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.guangxi.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const { decryedData } = checkRes.data;
        const res = await ctx.service.etaxFpdkLogin.guangxi.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.guangxi.login({ data: decryedData });
        if (res.errcode !== '0000') {
        ...
        // 更新客户端与服务端数电账号
        const opt = {
            city: decryedData.city || '广西',
            etaxName: decryedData.etaxName || 'guangxi',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
