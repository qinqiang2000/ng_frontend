...
export class GuizhouController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.guizhou.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const checkRes = await ctx.service.ntTools.checkRequestForLongLinkEtaxLogin();
        if (checkRes.errcode !== '0000') {
            ctx.service.log.info('guizhou sendShortMsg------------', checkRes);
            await this.responseJson(checkRes);
            return;
        }
        const { decryedData } = checkRes.data;

        const res = await ctx.service.etaxFpdkLogin.guizhou.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        // login 登录状态检测取消
        const res = await ctx.service.etaxFpdkLogin.guizhou.login({
            data: decryedData,
            needCheck: false
        });
        ...
        const opt = {
            city: decryedData.city || '贵州',
            etaxName: decryedData.etaxName || 'guizhou',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
