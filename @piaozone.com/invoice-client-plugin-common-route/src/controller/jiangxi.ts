...
export class JiangxiController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.jiangxi.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const checkRes = await ctx.service.ntTools.checkRequestForLongLinkEtaxLogin();
        if (checkRes.errcode !== '0000') {
            ctx.service.log.info('jiangxi sendShortMsg------------', checkRes);
            await this.responseJson(checkRes);
            return;
        }
        const { decryedData } = checkRes.data;

        const res = await ctx.service.etaxFpdkLogin.jiangxi.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        // login 登录状态检测取消
        const res = await ctx.service.etaxFpdkLogin.jiangxi.login({
            data: decryedData,
            needCheck: false
        });
        ...
        const opt = {
            city: decryedData.city || '江西',
            etaxName: decryedData.etaxName || 'jiangxi',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
