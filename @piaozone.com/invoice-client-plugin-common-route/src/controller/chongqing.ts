...
export class ChongqingController extends PluginBaseController {
    ...
    async stepOneLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.chongqing.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const res = await ctx.service.etaxFpdkLogin.chongqing.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        // login 登录状态检测取消
        const res = await ctx.service.etaxFpdkLogin.chongqing.login({
            data: decryedData,
            needCheck: false
        });
