...
export class XiamenController extends PluginBaseController {
    ...
    async stepOneLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.xiamen.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const { decryedData } = checkRes.data;
        const res = await ctx.service.etaxFpdkLogin.xiamen.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.xiamen.login({ data: decryedData });
        if (res.errcode !== '0000') {
