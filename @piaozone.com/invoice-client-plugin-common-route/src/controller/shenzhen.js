...
export class ShenzhenController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.shenzhen.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const checkRes = await ctx.service.ntTools.checkRequestForLongLinkEtaxLogin();
        if (checkRes.errcode !== '0000') {
            ctx.service.log.info('shenzhen sendShortMsg------------', checkRes);
            await this.responseJson(checkRes);
            return;
        }
        const { decryedData } = checkRes.data;

        const res = await ctx.service.etaxFpdkLogin.shenzhen.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        // login 登录状态检测取消
        const res = await ctx.service.etaxFpdkLogin.shenzhen.login({
            data: decryedData,
            needCheck: false
        });
        ...
        // 添加或更新服务端与客户端的电子税局账号
        const opt = {
            city: decryedData.city || '深圳',
            etaxName: decryedData.etaxName || 'shenzhen',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
