...
export class LiaoningController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.liaoning.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const checkRes = await ctx.service.ntTools.checkRequestForLongLinkEtaxLogin();
        if (checkRes.errcode !== '0000') {
            ctx.service.log.info('liaoning sendShortMsg------------', checkRes);
            await this.responseJson(checkRes);
            return;
        }
        const { decryedData } = checkRes.data;

        const res = await ctx.service.etaxFpdkLogin.liaoning.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        // login 登录状态检测取消
        const res = await ctx.service.etaxFpdkLogin.liaoning.login({
            data: decryedData,
            needCheck: false
        });
        ...
        // 添加或更新服务端与客户端的电子税局账号
        const opt = {
            city: decryedData.city || '辽宁',
            etaxName: decryedData.etaxName || 'liaoning',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
