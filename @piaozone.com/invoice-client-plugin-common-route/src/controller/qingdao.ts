...
export class QingdaoController extends PluginBaseController {
    ...
    async stepOneLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.qingdao.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const { decryedData } = checkRes.data;
        const res = await ctx.service.etaxFpdkLogin.qingdao.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.qingdao.login({ data: decryedData });
        if (res.errcode !== '0000') {
