...
export class DalianController extends PluginBaseController {
    ...
    async stepOneLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.dalian.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const res = await ctx.service.etaxFpdkLogin.dalian.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        // login 登录状态检测取消
        const res = await ctx.service.etaxFpdkLogin.dalian.login({
            data: decryedData,
            needCheck: false
        });
