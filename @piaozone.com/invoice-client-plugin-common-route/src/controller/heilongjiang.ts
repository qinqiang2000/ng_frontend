...
export class HeilongjiangController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.heilongjiang.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const { decryedData } = checkRes.data;
        const res = await ctx.service.etaxFpdkLogin.heilongjiang.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.heilongjiang.login({ data: decryedData });
        if (res.errcode !== '0000') {
        ...
        // 更新客户端与服务端数电账号
        const opt = {
            city: decryedData.city || '黑龙江',
            etaxName: decryedData.etaxName || 'heilongjiang',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
