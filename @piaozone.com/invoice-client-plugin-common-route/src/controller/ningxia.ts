...
export class NingxiaController extends PluginBaseController {
    async stepOneLogin() {
        const ctx = this.ctx;
        ...
        const res = await ctx.service.etaxFpdkLogin.ningxia.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const checkRes = await ctx.service.ntTools.checkRequestForLongLinkEtaxLogin();
        if (checkRes.errcode !== '0000') {
            ctx.service.log.info('ningxia sendShortMsg------------', checkRes);
            await this.responseJson(checkRes);
            return;
        }
        const { decryedData } = checkRes.data;

        const res = await ctx.service.etaxFpdkLogin.ningxia.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        // login 登录状态检测取消
        const res = await ctx.service.etaxFpdkLogin.ningxia.login({
            data: decryedData,
            needCheck: false
        });
        ...
        const opt = {
            city: decryedData.city || '宁夏',
            etaxName: decryedData.etaxName || 'ningxia',
            autoLogin: false,
            taxNo,
            companyName,
            loginAccountUid,
