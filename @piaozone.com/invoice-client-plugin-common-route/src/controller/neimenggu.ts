...
export class NeimengguController extends PluginBaseController {
    ...
    async stepOneLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.neimenggu.stepOneLogin({ data: decryedData });
    ...
    async sendShortMsg() {
        ...
        const { decryedData } = checkRes.data;
        const res = await ctx.service.etaxFpdkLogin.neimenggu.sendShortMsg({ data: decryedData });
        await this.responseJson(res);
    }
    ...
    async stepTwoLogin() {
        ...
        const res = await ctx.service.etaxFpdkLogin.neimenggu.login({ data: decryedData });
        if (res.errcode !== '0000') {
