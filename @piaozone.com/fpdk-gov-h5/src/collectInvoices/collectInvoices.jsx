...
class CollectInvoices extends React.Component {
    ...
    async checkInvoices(listData, startTime, checkIng) {
        ...
        for (let i = 0; i < listData.length; i++) {
            ...
            const { invoiceMoney = '', invoiceCode = '', invoiceNo = '', checkCode = '' } = curData;
            ...
            if (invoiceType !== '' && curData.status !== 1) {
                ...
                if (addedTax.indexOf(invoiceType) !== -1) {
                    if ((invoiceMoney !== '' || checkCode !== '') && invoiceCode !== '' && invoiceNo !== '' && invoiceDate != '') {
                        invoiceDate = moment(invoiceDate, 'YYYYmmDD').format('YYYY-mm-DD');

                        let tempKpje = parseFloat(invoiceMoney);

                        if (invoiceMoney !== '' && !isNaN(tempKpje)) {
                            tempKpje = tempKpje.toFixed(2);
                        } else {
                            tempKpje = '';
                        }

                        if (((invoiceType === 2 || invoiceType === 4 || invoiceType === 12 || invoiceType === 13) && tempKpje !== '') || checkCode !== '') {
    ...
    onScanFiles = () => {
        ...
        this.asyncScanFiles.startScan({
            ...
            stepUploadFinish: (res, fileInfo) => { // 每一步上传处理完成后的回调
                this.handlerUploadRes(res, fileInfo.id, 'scan');
            },
            uploadFinish: (res) => {
                message.destroy();
                if (res.errcode !== '0000') {
