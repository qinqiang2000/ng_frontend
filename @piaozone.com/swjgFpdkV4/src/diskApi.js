...
export async function getVersionApi(flag, url) {
    if(!url) {
        url = caOperateUrl;
    }
	...
	return await getApi(apiUrl, data);
}


export async function verifyPinApi(passwd, flag, alg, url){
    ...
    const res = await getApi(apiUrl, data);
...
export async function getClientHello(passwd, url, alg){
    if(!url) {
        url = caOperateUrl;
    }
...
export async function getClientAuthCode(passwd, serverPacket, url, alg){
    if(!url) {
        url = caOperateUrl;
    }
...
export async function getTaxInfoFromDisk(passwd, type, url, alg){   //71获取税号，27获取企业名称
    if(!url) {
        url = caOperateUrl;
    }
