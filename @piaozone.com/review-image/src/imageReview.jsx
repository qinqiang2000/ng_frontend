...
export default function ImageReview(props) {
    ...
    const isCutImage = !!(signType !== 'cover' && (inputInvoices.length > 1 || region) && getFileType(extName) === 'image') && showCutImage; // 是否用混贴展示
    ...
    let showFileUrl = snapshotUrl && showFileType === 'jpg' ? setWaterMark(snapshotUrl) : localUrl;
    if (showFileUrl) {
        showFileUrl = showFileUrl + `&originalFile=${originalFile}`
    }
    return (
