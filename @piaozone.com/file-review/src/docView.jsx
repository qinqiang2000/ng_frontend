...
export default function DocView(props) {
    const { fileUrl, angle, changeAngle, extName, method, onReviewFileUrl } = props;
    let staticUrl = window.staticUrl;

    const childRef = React.createRef();
    const clientInfo = clientCheck();
    const [realReviewUrl, setRealReviewUrl] = useState(null);
    const access_token = getUrlParam(fileUrl, 'access_token');

    useEffect(() => {
        async function onGetUrl() {
            const reviewFileUrl = await onReviewFileUrl({ reviewMethod: method, fileUrl, accessToken: access_token });
            setRealReviewUrl(reviewFileUrl);
        }
        onGetUrl();
    }, [fileUrl]);
    ...
    if ((getFileType(extName) === 'pdf') && (clientInfo.browser.chrome > 80 || clientInfo.browser.firefox > 80)) { // 高版本浏览器才走
        if (staticUrl?.indexOf('localhost') !== -1) {
            staticUrl = 'https://img.piaozone.com';
        }
