...
class ShowRelateBill extends React.Component {
    ...
    freshList = async(serialNo, pageNo = this.state.pageNo, pageSize = this.state.pageSize) => {
        ...
        const res = await this.props.getRelateBillInfo({ serialNo, pageNo, pageSize });
        if (res.errcode === '0000') {
            this.setState({
                loading: false,
                listData: res.data || [],
    ...
    render() {
        const colums = [
            { title: '序号', key: 'indexNum', align: 'left', width: 40, render: (v, r, i) => { return i + 1; } },
            { title: '报销单号', dataIndex: 'expenseNum', align: 'left', width: 90, render: (v) => { return v || '--'; } },
            { title: '报销人', dataIndex: 'expenseName', align: 'left', width: 74, render: (v) => { return v || '--'; } },
            { title: '报销时间', dataIndex: 'expenseTime', align: 'left', width: 120, render: (v) => { return v || '--'; } },
            { title: '报销事由', dataIndex: 'remark', align: 'left', width: 150, render: (v) => { return v || '--'; } }
        ];
...
ShowRelateBill.propTypes = {
    ...
    getRelateBillInfo: PropTypes.func.isRequired
};
