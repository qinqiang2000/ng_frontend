export function showDownloadMsi(opt = {}) {
    const {
        downloadUrl,
        upgradeFlag,
        onCancel,
    ...
    const boxEl = document.getElementById('dialog-download-msi');
    if (!boxEl) {
        const cssStr=`
        .dialog-download-msi {
            display: none;
        ...
        let description = '当前未安装扫描必须的组件JsScanner.msi，您可以下载安装后再使用';
        if (upgradeFlag) {
        ...
        <div class="dialog-content">
            <p>${description}</p>
            <a style="display: none" href="${downloadUrl}" id="downloadLink" target="_blank">download</a>
        ...
        // 确定下载按钮
        document.getElementById('comConfirmDownloadBtn').onclick = async function() {
        ...
        // 取消选择
        document.getElementById('cancelDownloadBtn').onclick = async function() {
            await onCancel();
            document.getElementById('dialog-download-msi').style.display = 'none';
        }
    } else {
