...
if(argv.init){
    ...
    fs.readdir(projectRootDir, async (err, curDirFiles) => {
        ...
        const allowTypes = ['antd-dva', 'antd-mobile-dva', 'egg-antd-dva', 'egg-antd-mobile-dva', 'lib', 'react-com'];
    ...
    if (argv.build) {
        ...
        webpackBuild(webpackConfig, argv);
    } else if(argv.dev) {
        const type = argv.type || '';
        console.log('webpack dev server is running...');
        const webpackDev = require('../lib/dev');
        let getWebpackConfig = require('../config/webpack.dev');
        let webpackConfig = getWebpackConfig(buildConfig);
        if(type === 'lib') { // 把自己的代码转换为lib文件
            getWebpackConfig = require('../config/webpack.lib');
            webpackConfig = getWebpackConfig(buildConfig, argv);
        }
        webpackDev(webpackConfig, buildConfig, argv);
    }
    ...
    if (installOnly || installLock) {
        ...
        let cmdDevFpyStr = '';
        if (cmdMiddleStr !== '') {
            cmdStr = 'npm install ' + cmdMiddleStr  + ' --registry=https://registry.npm.taobao.org --save';
            if (isProduction) {
                cmdStr += ' --production';
            }
            cmds.push(cmdStr);
        ...
        if (!isProduction) {
            if (cmdDevMiddleStr !== '') {
                cmdDevStr = 'npm install ' + cmdDevMiddleStr  + ' --registry=https://registry.npm.taobao.org --save-dev';
                cmds.push(cmdDevStr);
            }
        ...
        if (fpyPackages.length > 0 || fpyDevPackages.length >0 ) {
            ...
            cmds.push('npm install --registry=https://registry.npm.taobao.org');
        }
    }
